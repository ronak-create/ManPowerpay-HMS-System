...
export const bulkUploadEmployees = asyncHandler(async (req, res) => {
  ...
  for (const r of rows) {
    try {
      if (!r.name || !r.email || !r.mobile || !r.empCode) throw new Error('Missing required fields');
      const existing = await prisma.user.findFirst({ where: { OR: [{ email: r.email.toLowerCase() }, { mobile: r.mobile }] } });
      ...
      await prisma.$transaction(async (tx) => {
        const user = await tx.user.create({ data: { name: r.name, email: r.email.toLowerCase(), mobile: r.mobile, passwordHash: hash, role: 'employee' } });
        await tx.employee.create({ data: { userId: user.id, empCode: r.empCode, departmentId: dept?.id, siteId: site?.id, designation: r.designation, dateOfJoining: new Date(r.doj), annualCTC: Number(r.annualCTC) || 0 } });
      });
      results.created++;
    } catch (err) {
...
export const listEmployees = asyncHandler(async (req, res) => {
  ...
  const [employees, total] = await Promise.all([
    prisma.employee.findMany({ where, skip: Number(skip), take: Number(limit), include: { user: { select: { name: true, email: true, mobile: true } }, site: true, department: true, salaryTemplate: true }, orderBy: { createdAt: 'desc' } }),
...
export const getEmployee = asyncHandler(async (req, res) => {
  const emp = await prisma.employee.findUnique({ where: { id: req.params.id }, include: { user: { select: { name: true, email: true, mobile: true, isActive: true } }, site: true, department: true, salaryTemplate: { include: { components: { orderBy: { sequence: 'asc' } } } }, documents: true } });
...
export const createEmployee = asyncHandler(async (req, res) => {
  const { name, email, mobile, password, empCode, departmentId, siteId, salaryTemplateId, designation, dateOfJoining, annualCTC, dateOfBirth, gender, address, emergencyContact, emergencyPhone, pfAccountNo, esicNo, pan, aadhaarNo, uanNo, bankName, bankAccountNo, ifscCode } = req.body;
  ...
  const result = await prisma.$transaction(async (tx) => {
    const user = await tx.user.create({ data: { name, email: email.toLowerCase(), mobile, passwordHash: hash, role: 'employee' } });
    const emp = await tx.employee.create({ data: { userId: user.id, empCode, departmentId, siteId, salaryTemplateId, designation, dateOfJoining: new Date(dateOfJoining), annualCTC: Number(annualCTC) || 0, dateOfBirth: dateOfBirth ? new Date(dateOfBirth) : null, gender, address, emergencyContact, emergencyPhone, pfAccountNo, esicNo, pan, aadhaarNo, uanNo, bankName, bankAccountNo, ifscCode } });
    return { user, emp };
  });
...
export const updateEmployee = asyncHandler(async (req, res) => {
  ...
  if (!emp) throw new ApiError(404, 'Employee not found');
  const oldValue = { ...emp };
  const { name, mobile, departmentId, siteId, salaryTemplateId, designation, annualCTC, dateOfBirth, gender, address, emergencyContact, emergencyPhone, pfAccountNo, esicNo, pan, aadhaarNo, uanNo, bankName, bankAccountNo, ifscCode } = req.body;
  await prisma.$transaction([
    prisma.user.update({ where: { id: emp.userId }, data: { name, mobile } }),
    prisma.employee.update({ where: { id: req.params.id }, data: { departmentId, siteId, salaryTemplateId, designation, annualCTC: Number(annualCTC) || emp.annualCTC, dateOfBirth: dateOfBirth ? new Date(dateOfBirth) : emp.dateOfBirth, gender, address, emergencyContact, emergencyPhone, pfAccountNo, esicNo, pan, aadhaarNo, uanNo, bankName, bankAccountNo, ifscCode } })
  ]);
...
export const uploadDocument = asyncHandler(async (req, res) => {
  if (!req.file) throw new ApiError(400, 'No file uploaded');
  const { type } = req.body;
  await prisma.employeeDocument.create({ data: { employeeId: req.params.id, type, filePath: req.file.path } });
...
export const generateAppointmentLetter = asyncHandler(async (req, res) => {
  const employee = await prisma.employee.findUnique({ where: { id: req.params.id }, include: { user: { select: { name: true } } } });
  ...
  await prisma.employeeDocument.upsert({ where: { employeeId_type: { employeeId: employee.id, type: 'appointment_letter' } }, update: { filePath: relativePath, uploadedAt: new Date() }, create: { employeeId: employee.id, type: 'appointment_letter', filePath: relativePath } });
...
export const downloadAppointmentLetter = asyncHandler(async (req, res) => {
  ...
  const doc = await prisma.employeeDocument.findUnique({ where: { employeeId_type: { employeeId, type: 'appointment_letter' } } });
  if (!doc || !fs.existsSync(path.join(process.cwd(), doc.filePath))) throw new ApiError(404, 'Appointment letter not generated yet');
...
export const downloadRelievingLetter = asyncHandler(async (req, res) => {
  ...
  const employee = await prisma.employee.findUnique({ where: { id: employeeId }, include: { user: { select: { name: true } } } });
  if (!employee) throw new ApiError(404, 'Employee not found');
  if (!employee.dateOfLeaving) throw new ApiError(400, 'Employee has not been offboarded yet');
  const company = await prisma.company.findFirst();
  let doc = await prisma.employeeDocument.findUnique({ where: { employeeId_type: { employeeId, type: 'relieving_letter' } } });
  if (!doc || !fs.existsSync(path.join(process.cwd(), doc.filePath))) {
    ...
    await fs.promises.mkdir(path.dirname(filePath), { recursive: true });
    await fs.promises.writeFile(filePath, pdfBuffer);
    doc = await prisma.employeeDocument.upsert({ where: { employeeId_type: { employeeId: employee.id, type: 'relieving_letter' } }, update: { filePath: relativePath, uploadedAt: new Date() }, create: { employeeId: employee.id, type: 'relieving_letter', filePath: relativePath } });
  }
  res.download(path.join(process.cwd(), doc.filePath));
});
