...
import {
  LayoutDashboard, Users, Calendar, ClipboardList, LogOut, ChevronRight, Bell, Briefcase, Menu
} from 'lucide-react';
...
const navGroups = [
  {
    label: 'Overview',
    items: [{ to: '/supervisor', icon: LayoutDashboard, label: 'Dashboard', end: true }]
  },
  {
    label: 'Workforce',
    items: [
      { to: '/supervisor/attendance', icon: Calendar, label: 'Attendance Entry' },
      { to: '/supervisor/team', icon: Users, label: 'Team Overview' },
    ]
  },
  {
    label: 'HR',
    items: [{ to: '/supervisor/leaves', icon: Briefcase, label: 'Leave Approvals' }]
  },
];
    ...
    <>
      ...
      <div className="mx-3 mb-4 bg-white/8 rounded-xl p-3 flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <div className="text-white/40 text-[10px] font-medium">Supervisor</div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        <header className="h-16 bg-white border-b border-gray-100 flex items-center px-4 sm:px-6 gap-3 flex-shrink-0 shadow-sm">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-400 hidden sm:block">ManpowerPay HMS — Supervisor</p>
          </div>
