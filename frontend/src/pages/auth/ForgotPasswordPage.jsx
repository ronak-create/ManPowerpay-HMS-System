      ...
      <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md">
        ...
        {!sent ? (
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            />
            <button type="submit" className="w-full bg-primary text-white py-2.5 rounded-lg font-semibold">Send OTP</button>
          </form>
        ) : (
          <p className="text-green-600 font-medium">OTP sent! Redirecting...</p>
        )}
        <a href="/login" className="block text-center text-sm text-primary mt-4 hover:underline">Back to Login</a>
      </div>
    </div>
  );
}
