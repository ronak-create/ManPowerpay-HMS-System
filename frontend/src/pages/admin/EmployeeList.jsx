...
import { Plus, Search, Filter, Edit2, Power, FileText, Upload, Download, X, CheckCircle, AlertCircle, FileDigit } from 'lucide-react';
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (res.data.data.created > 0) {
        toast.success(`Successfully created ${res.data.data.created} employees`);
        onSuccess();
      }
    ...
    <Modal open={open} onClose={onClose} title="Bulk Employee Upload" size="lg">
      <div className="space-y-6">
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <ul className="text-xs text-blue-700 mt-1 space-y-1 list-disc ml-4">
              ...
              <li>Department and Site names must match exactly as configured in settings.</li>
            </ul>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-2xl py-10 px-4 bg-gray-50">
          ...
          <p className="text-sm text-gray-500 mb-4">Click to select or drag and drop Excel file (.xlsx)</p>
          <input
            type="file"
            accept=".xlsx"
        ...
        <div className="flex items-center justify-between pt-4 border-t">
          <button onClick={downloadTemplate} className="flex items-center gap-2 text-primary font-semibold hover:underline text-sm">
        ...
        {result && result.failed.length > 0 && (
          <div className="mt-4 border-t pt-4">
            <p className="text-sm font-bold text-red-600 mb-2">Failed Rows ({result.failed.length})</p>
...
export default function EmployeeList() {
  ...
  const generateLetter = async (id) => {
    setGenerating(id);
    try {
      await api.post(`/employees/${id}/appointment-letter`);
      toast.success('Appointment letter generated');
      fetchEmployees(); // to update UI if needed (though not strictly necessary yet)
    } catch (err) {
  ...
  const downloadLetter = async (id, empCode) => {
    try {
      const res = await api.get(`/employees/${id}/appointment-letter`, { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([res.data], { type: 'application/pdf' }));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `appointment_letter_${empCode}.pdf`);
      document.body.appendChild(link);
      link.click();
      link.remove();
    } catch (err) {
      toast.error('Download failed. Generate it first.');
    }
  };
              ...
              <tr key={emp.id} className="hover:bg-gray-50 transition">
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end gap-2">
                    <button
                      onClick={() => generateLetter(emp.id)}
                      disabled={generating === emp.id}
                      className="p-1.5 text-orange-600 hover:bg-orange-50 rounded"
                      title="Generate Appointment Letter"
                    >
                      {generating === emp.id ? <span className="w-4 h-4 border-2 border-orange-600 border-t-transparent rounded-full animate-spin block" /> : <FileDigit size={16} />}
                    </button>
                    <button
