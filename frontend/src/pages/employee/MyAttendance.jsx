...
export default function MyAttendance() {
  ...
  }, [authReady, currentDate, user?.employee?.id]);
  ...
  const statusLabels = {
    ...
    PL: "Paid Leave", WO: "Week Off", HO: "Holiday", LWP: "Loss of Pay",
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4 bg-white border rounded-lg px-4 py-2 shadow-sm">
          <button onClick={prevMonth} className="hover:text-primary transition">
            <ChevronLeft size={20} />
          </button>
          <span className="font-bold min-w-32 text-center">
            {format(currentDate, "MMMM yyyy")}
          </span>
          <button onClick={nextMonth} className="hover:text-primary transition">
              ...
              : calendarDays.map((day, i) => {
                  ...
                  return (
                    <div key={dayStr} className="aspect-square border rounded-xl flex flex-col items-center justify-center relative hover:shadow-md transition group cursor-default">
                      ...
                      {record?.status && <div className={`w-2 h-2 rounded-full ${color}`} />}
                      {record?.status && (
                        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 bg-white/95 flex flex-col items-center justify-center rounded-xl transition-all scale-95 group-hover:scale-100">
                          ...
                          {record.otHours > 0 && (
                            <span className="text-[10px] text-primary font-bold">+{record.otHours}h OT</span>
                          )}
                        </div>
                      )}
                    </div>
