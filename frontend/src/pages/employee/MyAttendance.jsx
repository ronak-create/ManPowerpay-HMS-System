...
export default function MyAttendance() {
  ...
  // BUG FIX: was user.id (userId) — backend expects employeeId and
  // throws 403 when userId !== employee.id in the ownership check.
  // Also add currentDate and employee ID as dependencies so it
  ...
  const fetchAttendance = async (empId) => {
    ...
    try {
      ...
      const res = await api.get(`/attendance/employee/${empId}?month=${month}&year=${year}`);
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border p-6">
          ...
          <div className="grid grid-cols-7 gap-2">
            {loading ? (
              ...
              return (
                <div key={dayStr}
                  className="aspect-square border rounded-xl flex flex-col items-center justify-center relative hover:shadow-md transition group cursor-default">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-primary rounded-2xl shadow-sm p-6 text-white">
            ...
            <div className="grid grid-cols-2 gap-2 text-[10px] font-bold uppercase">
              <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-green-400" /> Present</div>
              <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-red-400" /> Absent</div>
              <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-amber-400" /> Half Day</div>
              <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-blue-400" /> Paid Leave</div>
              <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-gray-400" /> Week Off</div>
              <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-indigo-400" /> Holiday</div>
            </div>
          </div>
        </div>
      </div>
